...
class ApiService {
  ...
  // Método para obtener los datos actualizados del proyecto y convertirlos en un objeto Perfil
  Future<Perfil> getProyecto(String id) async {
    ...
    if (response.statusCode == 200) {
      ...
      return Perfil.fromJson(
          proyectoData); // Suponiendo que tengas un constructor fromJson en tu clase Perfil
    } else {
  ...
  Future<List<Perfil>> getAllProyectos() async {
    ...
    if (response.statusCode == 200) {
      ...
      print('JSON recibido: $decoded'); // Para depuración

      // Asegurarse de que decoded contiene la clave 'mensaje' que es una lista
  ...
  Future<void> updateRecord(String id, int newRecord) async {
    try {
      // Obtén el perfil actual para verificar el récord existente
      final response = await http.get(Uri.parse('$baseUrl/get/$id'));
      if (response.statusCode == 200) {
        final perfil = Perfil.fromJson(json.decode(response.body));

        // Sólo actualizar si el nuevo récord es mayor que el existente
